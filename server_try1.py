...
def get_html(labels, sensorvalues):
    try:
       ...
       f.close()
    values = lux.split(',')
    return render_template('./try1.html', values=values, labels=labels, sensorvalues = sensorvalues)
...
def update_lux():
    ...
    try:
        ...
        f.close()
    return get_html(labels, sensorvalues):
                try:
                    f = open('sensor_data.csv', 'r')
                for row in f:
                    lux = row
